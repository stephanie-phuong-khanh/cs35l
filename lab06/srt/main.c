...
//function passed to each thread to execute
void* rayTrace(void* thread_index) {
    for(int px = (int)thread_index; px<width; px+=nthreads)
    {
        ...
        for(int py=0; py<height; ++py)
        {
            ...
            pixels[px][py][0] = scaled_color[0];
            pixels[px][py][1] = scaled_color[1];
            pixels[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
{
    nthreads = argc == 2 ? atoi( argv[1] ) : 0; //changes global variable
    ...
    subsample_dx = 
      halfSamples ? pixel_dx / ((double)halfSamples*2.0) : pixel_dx;
    subsample_dy = 
      halfSamples ? pixel_dy / ((double)halfSamples*2.0) : pixel_dy;
    ...
    for (int i=0; i<nthreads; i++) {
      ...
      if (ret) {
        fprintf(stderr, "Error creating thread: error %d\n", ret);
        exit(1);
      }
    }

    // wait for completion of spawned threads' tasks
    for (int i=0; i<nthreads; i++) {
      int ret = pthread_join(threads[i], NULL);
      if (ret) {
        fprintf(stderr, "Error joining thread: error %d\n", ret);
        exit(1);
      }
    }
